...
function Agreements({ ...props }) {
    servicesStore.readAgreements();
    let agreements = servicesStore.agreements;

    //alert("HI "+agreements);
    useEffect(() =>  {
       //alert("ADOPTION "+agreements);
    });

    return (<div> {agreements.title} - {agreements.statement} - <InnerHTML html={agreements.description}/>
           </div>);
}

export default withStyles(agreementsStyle)(observer(Agreements));
