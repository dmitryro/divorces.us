...
from rest_registration import utils
...
from users.decorators import run_async
from users.models import User
from utils.models import TaskLog
...
def send_activation_link(instance):
    ...
    try:
        ...
        log = Logger(log=f'WE FAILED SENDING ACTIVATION {e}')
        log.save()
...
def recover_profile(sender, instance, request, email,**kwargs):
    ...
    #contstruct the message
        # BODY='<html><head><style>.color{color:#FFAA44;}</style></head><body><div
        # class="color"><strong>USERNAME</strong> %s'%user.username
        # BODY+='</strong></div><br/><strong>EMAIL</strong> %s'%user.email
        # BODY+='</strong><br/><strong>MESSAGE</strong> %s'%mess
        # BODY+='</strong><br/><strong>PLEASE FOLLOW THE LINK TO RECOVER:
        # </strong> '
    ...
    # the HTML message, is best and preferred.
        ...
        server.sendmail(FROM, TO, msg)
        server.quit()
  #      log_new_user.send(sender='create_profile', message='created new user',
  #      level=1)
#        raise
#        ForceResponse(HttpResponseRedirect(reverse('success',kwargs={'code':
#        'success'})))
...
def new_account_notify(instance, email):
    ...
    #contstruct the message
        ...
        log = Logger(log=f'Something prevented us from sending {e} ')
        log.save()
