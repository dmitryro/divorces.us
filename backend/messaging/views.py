...
def DeleteMessageView(request):
    if request.method == 'GET':
        try:
            ...
            log.save()
            return Response({'messages':'error','exception':'message id is a mandatory'})

    elif request.method == 'POST':
        try:
            message_id = int(str(request.data['message_id']))
...
@renderer_classes((JSONRenderer,))
@permission_classes([AllowAny,])
def ReadMessageView(request):

    if request.method == 'GET':
        user = request.user
        message_id = int(str(request.params.get('message_id',0)))
        ...
        try:
            ...
            return Response({"messages":serializer.data, "total_unseen": total_unseen})

        except Exception as e:
            log = Logger(log=f"ERROR {e}")
            log.save()
            return Response({'messages':{'message':f'error - {e}'}})


    elif request.method == 'POST':
        user = request.user
        message_id = int(str(request.data.get('message_id',0)))
        m_id = int(message_id)
        ...
        try:
            ...
            log = Logger(log=f"ERROR - {e}")
            log.save()
            return Response({'messages':{'message':'error  '+str(e)}})
